...
export function StayList({ stays, byCity = false }) {
    ...
    const villas = [
        ...safeStays.filter(s => s?.type === 'villa').slice(0, ROW_SIZE),
        { isLinkCard: true, linkTo: '/search?type=villa' }
    ]
    const budgetStays = [
    ...
    return (
        <section className="stay-list-container">
            <StayCarouselRow title="Popular villas" stays={villas} itemsPerSlide={7} />
